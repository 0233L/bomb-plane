...
// 价格第 3 版（2026-08 AI 自对弈平衡）：依据 AI 自对弈使用率（旧价下 burst 4.3 次/局
// 过强、sonar/pro/doom 为 0 过弱）——热门涨、冷门降：
// 双发 3→4（过强涨价）、声呐 4→3、探测者 4→3（过弱降价）、毁灭菇 8→6（攒钱太久）。
// 价格第 4 版（2026-08）：声呐 3→4（AI 自对弈使用率 3.65-6.63 次/局爆热，且吸干
// 其他道具金币导致 pro/devour/doom 零使用 → 涨回 4 抑制）。
// AI 决策阈值与价格联动（见 aiDecideItem / DECIDE_DEFAULTS）。
// 道具使用 = 一次标准行动（steps +1，双发连射 +2）。
const ITEM_PRICES = {
  pro: 3,     // 探测者：3x3 区域内随机揭示 1 格真实内容（机身→机头→空格；区域全空则揭示整个区域）
  devour: 3,  // 吞噬者：3x3 区域内所有未揭示格变为「摧毁」（机头被摧毁 = 发现飞机）
  sonar: 4,   // 声呐脉冲：3x3 区域显示非空格数量（0~9）
  expose: 4,  // 无所遁形：对已揭示的机头使用，完整揭示整架飞机（10 格）
  burst: 4,   // 双发连射：一次行动揭示 2 格（只占一步）
  doom: 6     // 毁灭菇：十字 5 格揭示 + 相邻未揭示格冻结（施放者接下来 2 次行动不能碰）
};
...
// AI 道具决策：薄包装 ai.js 的 decideItem（决策逻辑单一来源在 ai.js，
// 自对弈脚本与服务器 AI 共用同一套阈值；调参实验直接覆盖 ai.js 的 opts）
function aiDecideItem(room, shots, size, myFrozen, pf) {
  return ai.decideItem(room.coins[1], shots, size, myFrozen, pf);
}
