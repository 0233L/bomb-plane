...
const testSrc = [
  ...
  '  }',
  '',
  '  // ===== 1. 声呐：区域内其他格 = 重新定位；再点第一击的定位格 = 确认 =====',
  '  lines.push("1. 声呐二次点击定位格确认");',
  '  enterPick("sonar");',
  '  pickItemCell(3, 4);',
  ...
  '  check("第一击定位格 = 3,4", state.pickFirstKey === "3,4");',
  ...
  '  check("定位格跟随更新为 3,5", state.pickFirstKey === "3,5");',
  '  check("重新定位不发送", emits.length === 0);',
  '  pickItemCell(3, 5); // 再点第一击的定位格 = 确认',
  '  check("二次点击定位格发送 useItem", emits.length === 1 && lastEmit().ev === "useItem");',
  ...
  '  // 已就绪时点新格 = 重新从第 1 格开始（已揭示格仍被拒）',
  '  emits = [];',
  '  enterPick("burst");',
  '  pickItemCell(2, 2);',
  '  pickItemCell(5, 5);',
  '  pickItemCell(3, 3);',
  '  check("就绪后点新格 = 重新选（从第 1 格开始）", state.pickCells.length === 1 && state.pickCells[0] === "3,3" && state.pickReady === false);',
  '  check("重新选不发送", emits.length === 0);',
  '  pickItemCell(1, 1);', // (1,1) 是已揭示的机头格：ready 态同样被拒，选区不变
  '  check("就绪后点已揭示格被拒", state.pickCells.length === 1 && state.pickCells[0] === "3,3" && state.pickReady === false);',
  '  check("被拒时不发送", emits.length === 0);',
  '',
  '  // ===== 5. 「确认使用」按钮仍然可用 =====',
  '  lines.push("5. 确认按钮");',
  '  emits = [];',
