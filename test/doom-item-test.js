...
async function main() {
  ...
  let roomA;
  {
    ...
    // 金币：10 - 10 + 十字内的收益（身 +1 / 头 +5 / 空 0）
    const gain = res.cells.reduce(function (n, c) {
      return n + (c.result === 'head' ? 5 : c.result === 'body' ? 1 : 0);
    }, 0);
  ...
  console.log('4. 选区整体拒绝 / 越界 / 中心含冻结');
  {
    ...
    // 快速攒钱：3 个机头（+15 → 23），比逐个揭示机身快
    const heads = knownB.map(function (p) { return [p.headRow, p.headCol]; });
    await aReveal(a, heads[0][0], heads[0][1]); // 1:0, 13
    await bReveal(b, 1, 1);
    await aReveal(a, heads[1][0], heads[1][1]); // 2:1, 18
    await bReveal(b, 2, 2);
    await aReveal(a, heads[2][0], heads[2][1]); // 3:2, 23
    await bReveal(b, 3, 3);
    const center = findCenter(knownB, heads);
    const res = await aUse(a, 'doom', { row: center.row, col: center.col },
      function (d) { return d.itemId === 'doom' && d.attacker === 0; }); // 4:3, coins 13+
    await bReveal(b, 4, 4); // 4:4
