...
const testSrc = [
  ...
  '  check("道具栏含金币经济", document.getElementById("rules-pane-props").textContent.indexOf("8 金币") !== -1);',
  ...
  '  state.sonarResults = [];',
  '',
  '  // ===== 10. 毁灭菇选区：十字轮廓框（4 臂各 3 条外缘线，中心无框线走 cell-pick 高亮） =====',
  ...
  '  check("中心格走 cell-pick 高亮", doomCls(5, 5).indexOf("cell-pick") !== -1);',
  ...
  '  check("3 金币买得起声呐（3 金币）", btn("sonar").disabled === false);',
  '  check("3 金币买不起毁灭菇（10 金币）", btn("doom").disabled === true);',
  '  check("3 金币买不起吞噬者（6 金币）", btn("devour").disabled === true);',
  '  check("探测者（2 金币）可买", btn("pro").disabled === false);',
